...
//  Anima-OpenGL
//
//  Created by Tim Jarratt on 11/27/13.
...
- (void)applicationDidFinishLaunching:(NSNotification *) aNotification {
    CFArrayRef cfDevices = [self retrieveAudioInputDevices];
    NSLog(@"appears that we have %lu input devices", CFArrayGetCount(cfDevices));

    for(int i = 0; i < CFArrayGetCount(cfDevices); ++i) {
        CFDictionaryRef deviceDict = CFArrayGetValueAtIndex(cfDevices, i);
        id name = CFDictionaryGetValue(deviceDict, CFSTR("deviceName"));
        id manufacturer = CFDictionaryGetValue(deviceDict, CFSTR("deviceManufacturer"));
        NSLog(@"%d: %@ by %@", i, name, manufacturer);
    }

    exit(0);
}

- (CFArrayRef) retrieveAudioInputDevices {
